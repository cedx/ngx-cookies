...
export class MapProvider implements CookieProvider {
  ...
  get cookie(): string {
    ...
    for (const [key, value] of this._map)
      if (value[1].expires && value[1].expires.getTime() <= now) this._map.delete(key);

    return [...this._map.entries()].map(([key, value]) => `${encodeURIComponent(key)}=${encodeURIComponent(value[0])}`).join('; ');
  }
