 ...
 * Builds the project.
 */
task('build:esm', () => src(['build/esm2015/**/*.js', '!**/ngx-cookies.js']).pipe(replace(/\/\/# sourceMappingURL=.*$/g, '')).pipe(dest('lib')));
task('build:src', () => _exec('ng', ['build']));
task('build:types', () => src(['build/**/*.d.ts', '!**/ngx-cookies.d.ts']).pipe(dest('lib')));
task('build', series('build:src', parallel('build:esm', 'build:types')));

/**
 * Uploads the results of the code coverage.
 */
task('coverage:fix', () => src('var/lcov.info').pipe(replace(/ngx-cookies\.ts([/\\])src/g, 'ngx-cookies.js$1src')).pipe(dest('var')));
